  ...
  <div style="max-width:680px;margin:0 auto;padding:40px 32px 80px;">
    <div style="margin-bottom:36px;">
      <h1 style="font-size:38px;font-weight:800;letter-spacing:-1px;">
        {{ $t('checkout.title') }} <span class="gradient-text">{{ $t('checkout.title2') }}</span>
      </h1>
    </div>

    <!-- Panier vide -->
    <div v-if="cart.items.length === 0" style="text-align:center;padding:60px 0;">
      <p style="color:#475569;margin-bottom:16px;">{{ $t('cart.empty') }}</p>
    ...
    <div v-else style="display:flex;flex-direction:column;gap:20px;">
      ...
      <div class="card" style="padding:28px;">
        <div style="font-size:11px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:1px;margin-bottom:18px;">{{ $t('checkout.summary') }}</div>
        ...
        <div style="border-top:1px solid rgba(255,255,255,0.07);margin-top:16px;padding-top:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px;">
          <span style="color:#e2e8f0;font-weight:600;">{{ $t('checkout.total') }}</span>
      ...
      <div class="card" style="padding:28px;">
        <div style="font-size:11px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:1px;margin-bottom:18px;">{{ $t('checkout.payment') }}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:18px;border-radius:16px;border:1px solid;"
          ...
          <div style="display:flex;align-items:center;gap:14px;">
            ...
            <div>
              <div style="font-size:14px;font-weight:700;color:#e2e8f0;">{{ $t('checkout.wallet') }}</div>
              <div style="font-size:13px;margin-top:3px;font-weight:600;"
                :style="canPay ? 'color:#34d399;' : 'color:#f87171;'">
                {{ balanceLoading ? '…' : `${balance.toFixed(2)} TND ${$t('checkout.available')}` }}
              </div>
            </div>
          </div>
          <div style="text-align:right;flex-shrink:0;">
            ...
            <div style="font-size:11px;color:#334155;margin-top:6px;">
              {{ $t('checkout.required') }} : <strong style="color:#e2e8f0;">{{ effectiveTotal.toFixed(2) }} TND</strong>
            </div>
          </div>
        </div>
        ...
        <div v-if="canPay && totalCashback > 0"
          ...
          <div style="font-size:12px;color:#c084fc;">
            {{ $t('checkout.cashback') }} <strong>{{ totalCashback.toFixed(2) }} TND</strong> {{ $t('checkout.cashback2') }}
          </div>
        </div>
      </div>
      ...
      <div class="card" style="padding:24px;">
        <div style="font-size:11px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:1px;margin-bottom:14px;">{{ $t('checkout.promo') }}</div>
        <div style="display:flex;gap:10px;align-items:center;">
          <input v-model="promoInput" type="text" :placeholder="$t('checkout.promo.ph')"
      ...
      <div v-if="!canPay && !balanceLoading"
        ...
        <div style="font-size:14px;font-weight:700;color:#f87171;margin-bottom:8px;">⚠️ {{ $t('checkout.insufficient') }}</div>
        <div style="font-size:13px;color:#64748b;line-height:1.6;margin-bottom:16px;">
          {{ $t('checkout.missing') }} <strong style="color:#fbbf24;">{{ (effectiveTotal - balance).toFixed(2) }} TND</strong> {{ $t('checkout.missing2') }}
        </div>
      ...
      >
        ...
        {{ loading ? $t('checkout.processing') : canPay ? $t('checkout.pay') : $t('checkout.insufficient') }}
      </button>
